...
struct Mask {
    ...
    void set(int b) { if(b<64)m0|=(1ULL<<b); else if(b<128)m1|=(1ULL<<(b-64)); else m2|=(1ULL<<(b-128)); }
    bool get(int b)const{ if(b<64)return(m0>>b)&1; else if(b<128)return(m1>>(b-64))&1; else return(m2>>(b-128))&1; }
    void set(int r,int c,int N){set(r*N+c);} bool get(int r,int c,int N)const{return get(r*N+c);}
    void clear(int b){if(b<64)m0&=~(1ULL<<b);else if(b<128)m1&=~(1ULL<<(b-64));else m2&=~(1ULL<<(b-128));}
    void clear(int r,int c,int N){clear(r*N+c);}
};
...
class ImprovedSolver {
    ...
    static double board_heuristic(const Board& b) {
            ...
            if (!vis[r][c]) {
                ...
                while (!q.empty()) { auto [cr,cc]=q.front(); q.pop_front(); sz++;
                    ...
                    for (int d=0;d<4;++d) { int nr=cr+DR[d],nc=cc+DC[d]; if (!b.in_bounds(nr,nc)||vis[nr][nc]) continue;
                        int nco=b.at(nr,nc).color(); if (!compatible_color(cco,nco)) continue;
        ...
        bool is_pure5=(b.level<=2), is_bomb_level=(b.level==4||b.level==5);
    ...
    static double drop_quality(const Board& b) {
        ...
        for (int c=0;c<b.N;++c) { int qp=b.queue_ptr[c];
            for (int i=0;i<7;++i) { if (qp+i>=1000) break; int v=b.drop_queue[c][qp+i];
                ...
                q+=adj; if (i<3) for (int j=i+1;j<3&&j<7;++j) {
    ...
    static int upper_bound_reachable(const Board& b, const Mask& mask, int r, int c, int fixed) {
        std::vector<std::vector<bool>> seen(b.N,std::vector<bool>(b.N));
        std::deque<std::pair<int,int>> q; int reachable=0;
        for (int d=0;d<4;++d) { int nr=r+DR[d],nc=c+DC[d];
            if (!b.in_bounds(nr,nc)||mask.get(nr,nc,b.N)||seen[nr][nc]) continue;
            if (!compatible_color(fixed,b.at(nr,nc).color())) continue;
            seen[nr][nc]=true; q.push_back({nr,nc}); ++reachable; }
        while (!q.empty()) { auto [cr,cc]=q.front(); q.pop_front();
    ...
    static int exact_best_one_step_score(const Board& b, int max_nodes) {
        ...
        std::function<void(Mask&,int,int,int)> dfs=[&](Mask& mask,int r,int c,int target) {
            ...
            int ub=upper_bound_reachable(b,mask,r,c,fixed);
            if (path_score((int)path.size()+ub)<=best) return;
            ...
            for (int i=0;i<nb_cnt;++i) { auto [nr,nc]=nb[i];
                int nf=(fixed==0&&b.at(nr,nc).color()!=0)?b.at(nr,nc).color():fixed;
                path.push_back({nr,nc}); Mask nm=mask; nm.set(nr,nc,b.N);
                dfs(nm,nr,nc,nf); path.pop_back(); if (nodes>max_nodes) return; }
        };
        for (int r=0;r<b.N&&nodes<=max_nodes;++r) for (int c=0;c<b.N&&nodes<=max_nodes;++c) {
            path.clear(); path.push_back({r,c}); Mask m; m.set(r,c,b.N);
            int co=b.at(r,c).color(); dfs(m,r,c,co==0?0:co); }
        return best;
    }
    ...
    static std::pair<int,std::vector<std::pair<int,int>>> exact_best_with_path(const Board& b, int max_nodes) {
        ...
        std::function<void(Mask&,int,int,int)> dfs=[&](Mask& mask,int r,int c,int target) {
            ...
            if ((int)path.size()>=2) { int sc=path_score(b,path);
                ...
                int ub=upper_bound_reachable(b,mask,r,c,fixed);
                if (path_score((int)path.size()+ub)<=best_score) return; }
            std::pair<int,int> nb[4]; int nb_cnt=0;
            for (int d=0;d<4;++d) { int nr=r+DR[d],nc=c+DC[d];
                if (!b.in_bounds(nr,nc)||mask.get(nr,nc,b.N)) continue;
                if (!compatible_color(fixed,b.at(nr,nc).color())) continue; nb[nb_cnt++]={nr,nc}; }
...
public:
    std::vector<std::pair<int,int>> solve(const Board& board) {
        ...
        switch(board.level) {
            ...
            case 4: cur_cache_limit_=80000;cur_keep1_=120;cur_keep2_=60;cur_keep3_=12;mxc_weight_=15.0;dq_weight_=0.8;bh_weight_=0.02;step3_dq_weight_=0.55;step3_bh_weight_=0.025;surv_mul_=1.0;cur_beam_w_=0;cur_beam_d_=0;break;
            ...
            default:cur_cache_limit_=60000;cur_keep1_=200;cur_keep2_=100;cur_keep3_=20;mxc_weight_=15.0;dq_weight_=0.8;bh_weight_=0.02;step3_dq_weight_=0.55;step3_bh_weight_=0.025;surv_mul_=1.0;cur_beam_w_=8;cur_beam_d_=4;
        }

        struct Raw { std::vector<std::pair<int,int>> path; int now; int nxt; };
        std::vector<Raw> raws; raws.reserve(16384);
        ...
        std::function<void(Mask&,int,int,int)> dfs=[&](Mask& mask,int r,int c,int target) {
            ...
            if ((int)path.size()>=2) { if ((int)path.size()>best_len) best_len=(int)path.size();
                int ns=path_score(board,path); Board nb=board.preview(path); auto h=board_hash(nb);
            ...
            int ub=upper_bound_reachable(board,mask,r,c,fixed);
            if ((int)path.size()+ub<=best_len) return;
            ...
            for (int i=0;i<nb_cnt;++i) { auto [nr,nc]=nb_arr[i];
                int nf=(fixed==0&&board.at(nr,nc).color()!=0)?board.at(nr,nc).color():fixed;
                path.push_back({nr,nc}); Mask nm=mask; nm.set(nr,nc,board.N);
                dfs(nm,nr,nc,nf); path.pop_back(); if (nodes>lmt) return; }
        };
        ...
        for (auto [r,c] : starts) { path.clear(); path.push_back({r,c});
            Mask m; m.set(r,c,board.N); dfs(m,r,c,board.at(r,c).color()==0?0:board.at(r,c).color()); if (nodes>lmt) break; }

        if (raws.empty()) { for (int r=0;r<board.N;++r) for (int c=0;c<board.N;++c) {
            path.clear(); path.push_back({r,c}); Mask m; m.set(r,c,board.N);
            dfs(m,r,c,board.at(r,c).color()==0?0:board.at(r,c).color()); if (nodes>lmt) break; } }
        ...
        for (int i=0;i<k1;++i) { double val=cands[i].v2;
            if (i<k2&&!tdead()) { auto [nsc,np]=exact_best_with_path(cands[i].next_b,cmax);
                if (np.size()>=2) { Board tb=cands[i].next_b.preview(np); auto th=board_hash(tb);
        ...
        if (!tfast() && cur_beam_w_ > 0) { int bs_cnt=std::min(3,(int)scored.size());
            for (int j=0;j<bs_cnt;++j) { double beam_val=beam_evaluate(cands[scored[j].idx].next_b,cur_beam_w_,cur_beam_d_); scored[j].val+=beam_val*0.3; }
            std::sort(scored.begin(),scored.end(),[](auto&a,auto&b){return a.val>b.val;}); }
        int sel = 0;
...
int ImprovedSolver::cur_cache_limit_=60000;int ImprovedSolver::cur_keep1_=200;
int ImprovedSolver::cur_keep2_=100;int ImprovedSolver::cur_keep3_=20;
double ImprovedSolver::mxc_weight_=15.0;double ImprovedSolver::dq_weight_=0.8;
double ImprovedSolver::bh_weight_=0.02;double ImprovedSolver::step3_dq_weight_=0.55;
double ImprovedSolver::step3_bh_weight_=0.025;double ImprovedSolver::surv_mul_=1.0;
int ImprovedSolver::cur_beam_w_=8;int ImprovedSolver::cur_beam_d_=4;

class LocalJudge {
    int level_, N_; Board board_; int step_=0,score_=0,invalid_streak_=0;bool done_=false;
...
public:
    LocalJudge(int level,int seed,int N):level_(level),N_(N){board_=Board(N);board_.level=level;
        board_.drop_queue.assign(N,std::vector<int>(1000));board_.queue_ptr.assign(N,0);
    ...
    bool validate(const std::vector<std::pair<int,int>>&path,std::string&reason)const{
        if(path.size()<2){reason="len<2";return false;}std::vector<std::vector<bool>>used(N_,std::vector<bool>(N_));
        int anchor=0;for(size_t i=0;i<path.size();++i){auto[r,c]=path[i];if(r<0||r>=N_||c<0||c>=N_){reason="OOB";return false;}
            if(used[r][c]){reason="dup";return false;}used[r][c]=true;int color=board_.at(r,c).color();
            if(color!=0){if(anchor==0)anchor=color;else if(anchor!=color){reason="color";return false;}}
            if(i>0){auto[pr,pc]=path[i-1];if(std::abs(pr-r)+std::abs(pc-c)!=1){reason="4conn";return false;}}}return true;}
    bool play(const std::vector<std::pair<int,int>>&path){if(done_||step_>=MAX_STEPS){done_=true;return false;}
        std::string reason;if(!validate(path,reason)){invalid_streak_++;step_++;if(invalid_streak_>=3)done_=true;return!done_;}
        invalid_streak_=0;int gained=path_score(path.size());
        if(level_>=4){std::vector<std::vector<bool>>in_path(N_,std::vector<bool>(N_)),exploded(N_,std::vector<bool>(N_));
            for(auto[r,c]:path)in_path[r][c]=true;for(auto[r,c]:path){if(!board_.at(r,c).is_bomb())continue;
                for(int dr=-1;dr<=1;++dr)for(int dc=-1;dc<=1;++dc){int nr=r+dr,nc=c+dc;
                    if(nr>=0&&nr<N_&&nc>=0&&nc<N_&&!in_path[nr][nc]&&!exploded[nr][nc]){exploded[nr][nc]=true;gained+=10;}}}}
        score_+=gained;step_++;std::vector<std::vector<bool>>to_remove(N_,std::vector<bool>(N_));
        for(auto[r,c]:path)to_remove[r][c]=true;if(level_>=4)for(auto[r,c]:path){if(!board_.at(r,c).is_bomb())continue;
            for(int dr=-1;dr<=1;++dr)for(int dc=-1;dc<=1;++dc){int nr=r+dr,nc=c+dc;
        ...
        if(board_.is_deadlocked()||step_>=MAX_STEPS)done_=true;return!done_;}
    void print_board()const{for(int r=0;r<N_;++r){for(int c=0;c<N_;++c){int v=board_.at(r,c).value;if(v>=0)std::cout<<' ';std::cout<<v<<' ';}std::cout<<"\n";}}
};

int main(){std::ios::sync_with_stdio(false);std::cin.tie(nullptr);
    struct{int lv,sd,N;const char*nm;}lvs[]={{1,42,10,"Level 1"},{2,42,10,"Level 2"},{3,42,10,"Level 3"},{4,42,10,"Level 4"},{5,42,12,"Level 5"}};
    ImprovedSolver solver;int total=0;
    for(auto&lc:lvs){LocalJudge judge(lc.lv,lc.sd,lc.N);std::cout<<"\n=== "<<lc.nm<<" ===\n";judge.print_board();
        int prev=0;while(!judge.done()){auto path=solver.solve(judge.board());if(path.size()<2)path={{0,0},{0,1}};
            std::string r;bool v=judge.validate(path,r);int s=judge.step();bool c=judge.play(path);
            int g=judge.score()-prev;prev=judge.score();std::cout<<"S"<<s+1<<": len="<<path.size();
            if(!v)std::cout<<" [INV:"<<r<<"]";else std::cout<<" [+"<<g<<"]";std::cout<<"\n";if(!c)break;}
        std::cout<<"Score: "<<judge.score()<<"\n";total+=judge.score();}
    std::cout<<"\nTOTAL: "<<total<<std::endl;return 0;}
