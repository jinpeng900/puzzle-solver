...
static void init_default_params() {
    g_params[1] = {900000, 200, 80, 13, 65000, 21.0, 1.15, 0.02, 0.45, 0.0, 0.4, 8, 5, 15, 10, 0.0, 0.0, 0.91, 0.99, 11000, 5};
    g_params[2] = {1000000, 180, 75, 17, 60000, 19.0, 0.85, 0.025, 0.5, 0.035, 0.9, 10, 3, 0, 10, 0.8, 0.25, 0.82, 0.99, 12500, 3};
    g_params[3] = {350000, 140, 60, 8, 65000, 22.5, 0.85, 0.03, 0.3, 0.015, 1.0, 0, 0, 50, 9, 0.4, 0.0, 0.83, 0.96, 8500, 6};
    g_params[4] = {600000, 60, 95, 17, 75000, 15.0, 0.5, 0.02, 0.65, 0.02, 1.0, 6, 6, 80, 6, 0.15, 0.0, 0.87, 0.98, 11500, 3};
    g_params[5] = {800000, 90, 20, 5, 35000, 20.5, 0.8, 0.025, 0.95, 0.03, 0.7, 0, 0, 50, 7, 0.45, 0.15, 0.94, 0.97, 9500, 6};
}
...
class ImprovedSolver {
    ...
    static std::vector<std::pair<int,int>> extend_path(const Board& b, const std::vector<std::pair<int,int>>& path) {
        if (path.size() < 2) return path;
        int N = b.N;
        std::vector<std::pair<int,int>> result = path;
        std::vector<std::vector<bool>> used(N, std::vector<bool>(N, false));
        for (auto [r,c] : path) used[r][c] = true;

        int target = b.at(path[0].first, path[0].second).color();
        if (target == 0) {
            for (auto [r,c] : path) { int co = b.at(r,c).color(); if (co != 0) { target = co; break; } }
        }
        ...
        while (extended) {
            ...
            for (int d = 0; d < 4; d++) {
                int nr = r + DR[d], nc = c + DC[d];
                if (!b.in_bounds(nr, nc) || used[nr][nc]) continue;
                if (!compatible_color(target, b.at(nr, nc).color())) continue;
                result.insert(result.begin(), {nr, nc});
                used[nr][nc] = true;
                extended = true; break;
            }
        }
