...
// ============== IMPROVED SOLVER (from 1.cpp, optimized) ==============
...
// ============== LOCAL JUDGE ==============
class LocalJudge {
    int level_, N_, step_=0, score_=0, invalid_streak_=0;
    Board board_;
    bool done_=false;
...
public:
    LocalJudge(int level, int seed, int N) : level_(level), N_(N) {
        ...
        for (int c=0;c<N;++c) for (int i=0;i<1000;++i) (*board_.drop_queue)[c][i]=gen_block(rng,level);
        std::mt19937 rng_board(seed ^ 0x9E3779B9);
        for (int r=0;r<N;++r) for (int c=0;c<N;++c) board_.at(r,c).value=gen_block(rng_board,level);
    }
    ...
    bool validate(const std::vector<std::pair<int,int>>& path, std::string& reason) const {
        ...
        for (size_t i=0;i<path.size();++i) {
            ...
            if (i>0) { auto [pr,pc]=path[i-1]; if (std::abs(pr-r)+std::abs(pc-c)!=1) { reason="4-conn"; return false; } }
        }
        return true;
    }
    ...
    bool play(const std::vector<std::pair<int,int>>& path) {
        ...
        std::string reason; bool valid=validate(path,reason);
        if (!valid) { invalid_streak_++; step_++; if (invalid_streak_>=3) done_=true; return !done_; }
        invalid_streak_=0;
        int gained=path_score(path.size());
        if (level_>=4) {
            std::vector<std::vector<bool>> in_path(N_,std::vector<bool>(N_)), exploded(N_,std::vector<bool>(N_));
            for (auto [r,c]:path) in_path[r][c]=true;
            for (auto [r,c]:path) {
        ...
        for (int c=0;c<N_;++c) {
            std::vector<Cell> remaining;
            for (int r=0;r<N_;++r) if (!to_remove[r][c]) remaining.push_back(board_.at(r,c));
            int empty=N_-(int)remaining.size();
            for (int i=0;i<empty;++i) board_.at(i,c).value=(*board_.drop_queue)[c][board_.queue_ptr[c]++];
            for (int i=0;i<(int)remaining.size();++i) board_.at(empty+i,c)=remaining[i];
        }
...
int main(int argc, char** argv) {
    ...
    std::vector<std::pair<int,std::pair<int,const char*>>> levels = {
        {1,{seed,"L1"}},{2,{seed,"L2"}},{3,{seed,"L3"}},{4,{seed,"L4"}},{5,{seed,"L5"}}
    };
    for (auto& [lv,info] : levels) {
        int N=(lv==5)?12:10;
        LocalJudge judge(lv, info.first, N);
        while (!judge.done()) {
            auto path = solver.solve(judge.board());
            if (path.size()<2) path={{0,0},{0,1}};
            if (!judge.play(path)) break;
        }
        std::cerr << info.second << ": " << judge.score() << " ";
        total += judge.score();
    }
    std::cout << total << "\n";
    return 0;
