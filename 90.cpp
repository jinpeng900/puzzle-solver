...
// 修复位掩码越界 BUG：支持到 192 位，完美覆盖 12x12 (144) 棋盘
struct Mask {
...
class ImprovedSolver {
    ...
    // 混合启发式：纯5色/万能块用6.cpp公式，炸弹关卡融合囤积流
        ...
        for (int i=0;i<k1;++i) {
            ...
            if (i<k2&&!tdead()) {
                ...
                if (np.size()>=2) {
                    ...
                    val=(double)cands[i].now+(double)nsc+(double)tsc+drop_quality(tb)*step3_dq_weight_+board_heuristic(tb, step_count_ + 2)*step3_bh_weight_;
                }
            }
