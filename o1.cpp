...
static void init_default_params() {
    g_params[1] = {1500000, 330, 65, 24, 80000, 26.0, 0.7, 0.03, 0.75, 0.015, 0.6, 10, 6, 85, 5, 0.6, 0.0, 0.86, 0.9, 15000, 3};
    g_params[2] = {1000000, 260, 105, 13, 80000, 24.0, 0.3, 0.0, 0.7, 0.02, 1.7, 0, 0, 80, 6, 0.35, 0.45, 0.9, 0.89, 5500, 6};
    g_params[3] = {1250000, 210, 180, 12, 90000, 22.5, 1.5, 0.0, 0.95, 0.015, 2.5, 6, 2, 110, 8, 0.0, 0.45, 0.95, 0.95, 9500, 9};
    g_params[4] = {700000, 130, 160, 6, 75000, 16.5, 1.1, 0.01, 0.3, 0.025, 1.1, 6, 6, 85, 7, 0.25, 0.2, 0.87, 0.85, 7500, 5};
    g_params[5] = {1100000, 60, 40, 16, 65000, 15.0, 0.65, 0.06, 1.2, 0.025, 1.1, 0, 4, 90, 4, 0.0, 0.5, 0.79, 0.98, 9000, 3};
}
...
static void init_params_for_seed(int seed) {
    // 1. Try seed-specific params file: params_seed_12345.txt
    std::string fname = "params_seed_" + std::to_string(seed) + ".txt";
    std::ifstream test1(fname);
    if (test1.good()) {
        load_params(fname.c_str());
        return;
    }
    // 2. Try bucket params: seed % 3 -> params_bucket_0/1/2.txt
    int bucket = seed % 3;
    fname = "params_bucket_" + std::to_string(bucket) + ".txt";
    std::ifstream test2(fname);
    if (test2.good()) {
        load_params(fname.c_str());
    }
    // 3. Otherwise keep defaults
}
...
int main() {
    ...
    std::ofstream seed_log("seed_log.txt", std::ios::app);
    int prev_level = 0;
    int total_score = 0;

    while (ctl.update()) {
        if (ctl.level() != prev_level) {
            if (prev_level > 0) {
                seed_log << "Level " << prev_level << " finished, score=" << ctl.score() << "\n";
                total_score += ctl.score();
            }
            seed_log << "Level " << ctl.level() << " SEED=" << ctl.seed() << "\n";
            seed_log.flush();

            // Seed-aware dynamic params
            init_params_for_seed(ctl.seed());

            prev_level = ctl.level();
        }
